...
| 🦇 **Batch Format**           | `BAT/Video`    | Pads an image + mask (+ optional control video) batch to a frame count the target video model accepts — WAN/Hunyuan 4k+1, LTX/Cosmos 8k+1, MiniMax H3 17k+5 — or to a specific count, and returns the padding so 🦇 Batch Crop can trim it back after generation. Replaces *Video Batch Format* and *WAN Batch Frame Format*; old workflows migrate through the popup. |
| 🦇 **Video Loader**           | `BAT/video`    | Standalone video reader. Type a path (with directory autocomplete), then set the range on a dual-handle trim slider with in/out thumbnails. Decodes through the first backend that works — decord, OpenCV, then `torchvision.io` — and emits **IMAGE + MASK + AUDIO + frame_rate**: the mask carries a source's alpha where it has one, and the audio is cut to the same trim window, so a trimmed clip's sound stays in sync without a second node. Decoded ranges are cached in a **byte**-bounded LRU (4 GiB) rather than a 4-entry one — four cached 500-frame 1080p float32 batches is ~50 GB, so an entry count said nothing about memory — which makes the common "re-run with the same trim window" free. The node face reports what the load will actually yield (bit depth, alpha, audio), because whether a `.mov` is 8- or 10-bit and whether it has alpha decides which outputs are worth wiring up and is invisible otherwise. In-node HTML5 preview with a scrubber and frame stepping so artists can verify the range before queuing. |
...
| Display name                  | Category       | What it does |
|-------------------------------|----------------|---|
| 🦇 **Bypass Switch**          | `BAT/workflow` | Named toggles that bypass / un-bypass saved selections of nodes, subgraphs and backdrops. Not wired into anything — it has no inputs, no outputs and never executes; it *manages* the graph rather than being part of it. For handing someone a template they can drive without knowing which twelve nodes to Ctrl+B. |

#### Bypass Switch

Drop one on the canvas. It starts empty, with a single **＋ Add first toggle…**
button.

1. Press it. The canvas goes into **selection mode** — a banner appears at the
   top of the screen and the current selection is cleared.
2. Click / shift-click / rubber-band whatever belongs together: nodes, subgraph
   nodes, backdrops.
3. Press <kbd>Enter</kbd> to confirm (<kbd>Esc</kbd> cancels and restores what
   you had selected). Name it, optionally drop it into an exclusive group.

You get a toggle on the node body. Clicking it sets every node in that
selection to **Always** (mode 0) or **Bypass** (mode 4). **✎ Edit toggles…**
opens a dialog to rename, reorder (▲▼), regroup, re-select and delete.

The behaviour that is worth knowing before you build a template on it:

- **Toggles are independent by default.** Put several in the same *exclusive
  group* and they draw as one dropdown — `Mode: Video` — where only one can be
  on. Better on a canvas widget than fake radio buttons, and the mutual
  exclusion is visible rather than something you find out by clicking.
- **A backdrop is remembered as a backdrop.** The nodes a toggle controls are
  whatever is inside that backdrop *at the moment you click it*, so anything
  you drag in later is covered with no re-capture. Loose nodes are remembered
  by id.
- **Opening a workflow rebuilds the toggles but does not touch node modes.**
  The bypass state is already in the workflow JSON, so re-asserting it would be
  redundant at best — and at worst it would silently undo hand-bypassing done
  on purpose. Right-click ▸ **Re-apply all toggles to the graph** when you do
  want the switch to be authoritative.
- **On is unconditional.** A toggle switched on forces mode 0, including on a
  node someone had muted. A toggle that sometimes left a node switched off
  would not be worth having in a template.
- **Deleting a toggle leaves its nodes alone.** It removes the switch, not the
  bypass.
- **Missing and duplicated targets are flagged.** A toggle whose targets were
  deleted draws a ⚠ and the edit dialog offers a one-click clean-up. Ids are
  *not* remapped on paste, so copying the switch together with its targets
  gives you a copy that still drives the originals — that case is detected (two
  switches sharing preset ids can only be a copy) and the newer node is marked
  and warned about. Re-select its toggles.

State lives in one hidden JSON `presets` widget, so it saves into the workflow,
survives undo/redo (which in ComfyUI is a full graph reload, and so only keeps
what actually serialises), and is readable in the `.json` if a template ever
needs hand-repair.
...
A sidebar panel (bat icon → **BAT Profiler**) that instruments *every* node in
the graph — not just this pack's — and answers the two questions you ask when a
workflow misbehaves: **which node ate the memory** and **which node ate the
clock**.

Per node it records wall time, RSS delta and peak, VRAM delta and peak, payload
bytes in and out, and disk I/O. Rank the list by any of those, click a row to
select and centre that node on the canvas. Above the list, RAM and VRAM are
charted live against the machine's real ceilings — a fixed ceiling rather than
an auto-scaled axis, so "how much headroom did I actually have" is readable at a
glance.

**It is off until you switch it on**, and the switch is per browser, not per
server. This is one shared process; an artist who never opens the panel runs the
original code path, because the arming gate sits at the top of `execute_async`
and their prompts never enter the instrumented branch. Your choice is
remembered, so a browser that was recording before a crash comes back recording.

**History is per workflow.** Runs are bucketed by workflow path, so five open
tabs each see only their own profiles — the analysis of a WAN render never
appears in an unrelated comp graph. Records are written to `localStorage` as
they stream in rather than at run end, which is the point: the backend's history
...
straight into a chat or a ticket. It carries the machine's ceilings, the memory
trajectory, which nodes peaked, which nodes kept memory, and — after a crash —
which node was running when the process died.

That last one is the reason the button exists. **An OOM kill leaves no error
behind**: the kernel SIGKILLs the process, nothing is raised, no
`execution_error` is sent, the websocket just stops. So the report can never be
produced by the thing that died. It is reconstructed afterwards in the browser
from records that were streamed to `localStorage` before the lights went out,
and three details make it diagnostic rather than a data dump:

- **It names the node that was running, not the last one that finished.** The
  culprit never completes, so it has no record of its own. The panel tracks it
  separately from core's `executing` event; the last one with no matching
  completion is the suspect.
- **A run left at `running` that neither the page nor the backend considers live
  is relabelled `lost`** — that state *is* the OOM signature, and it is what
  lets the report state a verdict.
- **Every figure is against its ceiling.** "18 GB VRAM" means nothing without
  "of 24 GB".

Peak and held get separate tables on purpose: one node peaking high and twenty
nodes each keeping half a gigabyte are different bugs with opposite fixes.

If `navigator.clipboard` is unavailable — which is the norm, since it needs a
secure context and ComfyUI is usually reached over plain `http://` on a LAN
address — it falls back to a hidden textarea, and then to showing the text for
manual selection.
...
| Column | Meaning |
|---|---|
| **Time** | Wall time inside the node. |
| **VRAM peak** | High-water allocator VRAM while the node ran — the number that decides whether you OOM. Exact, via `max_memory_allocated`. |
| **VRAM held** | Still allocated *after* the node returned. Positive values accumulate down the graph and are why a long workflow dies twenty nodes later. |
| **RAM peak** | High-water process RSS. Sampled at 5 Hz, so a spike that opens and closes inside 200 ms can be missed — deltas are exact, peaks are observed. |
| **Data in / out** | Bytes of tensor payload crossing the node. |
| **Disk I/O** | Syscall-level (`read_chars`/`write_chars`). Block-layer counters under-report badly on NFS, which is where the plates live. |

Cached and skipped nodes are listed but excluded from ranking by default.
ComfyUI does not merely short-circuit a cached node — `ExecutionList` never adds
it, so `execute()` is never called for it; those rows are reconstructed at run
end by diffing the prompt against what was actually visited.

### Where it hooks

Two module-level functions in `execution.py` are rebound, both called as globals
from inside `PromptExecutor` — no source patching, no subclassing:

- **`execute()`** — the per-node boundary.
- **`get_output_data()`** — the single normal-path producer of a node's outputs.
  Measuring payload here rather than reading `caches.outputs` back means the
  profiler never touches the output cache, so it cannot perturb LRU recency or
  the RAM-pressure cache provider. It also hands over `input_data_all` for free.

A `ProgressHandler` would have been the official extension point (ETC_Farm uses
one), but it never sees a node's payloads, which is half of what the panel is
for.

### Caveats worth knowing

- **CUDA is asynchronous.** Without a sync at each node boundary, GPU work is
  billed to whichever later node happens to block. *GPU sync* (on by default)
  makes attribution correct; it moves where work is billed rather than adding
  work.
- **Peak VRAM needs a counter reset** per node. That counter is process-global,
  but nothing in ComfyUI core reads it, so resetting is safe.
- **Async and lazy nodes re-enter `execute()`.** Records accumulate: time sums,
  peaks take a max, deltas span first entry to last exit. For a genuinely async
  node the wall time is not the work's duration, and the row says so.

### Cost

Measured on a 29-node graph of deliberately trivial nodes, so the fixed cost is
visible rather than lost in noise:

```
unarmed            179.9 ms
armed              201.1 ms      +0.73 ms per node
  of which GPU sync              +0.24 ms per node
  probes + payload               +0.74 ms per node
```

Read it as a **per-node constant of roughly 1 ms**, not as the +12% that
benchmark implies. On a real graph, where a node takes tenths of a second to
minutes, it disappears. An unarmed client pays nothing measurable — one identity
check per node.
...
ComfyUI stops you zooming out at 10%, which on a large comp graph is not far
enough to see the whole thing. `web/bat_canvas_zoom.js` lowers that floor —
default **5%**, i.e. twice as far out, adjustable down to 1% under
*Settings → 🦇 BAT → Canvas*.

There is one number behind every zoom gesture in the frontend:
`canvas.ds.min_scale`, which litegraph's `DragAndScale.changeScale()` clamps
against. The wheel, Alt+`-`, the View menu's *Zoom Out*, the zoom-percentage
box and the two-finger pinch all end up there, so moving that one field covers
all of them. Zooming *in* is untouched — `max_scale` stays at litegraph's 10×.

Far out, core's own level-of-detail rendering draws nodes as plain boxes and
the DOM-widget layer stops laying out widgets too small to read. That is the
point of an overview, and it is also why the extra range stays cheap: more
graph area costs box fills, not text and HTML.

> Not to be confused with `bat_zoom_control.js` below, which is the
> display-only zoom *inside* a node's preview canvas. This one is the graph.
...
what it walks, since a profiler that pins output tensors would cause the OOM it
diagnoses), the arming gate, record accumulation across re-entries, the CUDA
probes, and that the `execution.py` signatures it reads positionally are still
the ones upstream ships. `verify_profiler_e2e.py` drives a real server through a
real prompt, which is the only thing that proves the hooks actually fire:

```bash
./env/bin/python main.py --port 8791 --listen 127.0.0.1 &
python tests/verify_profiler_e2e.py
python tests/bench_profiler.py      # overhead, armed vs unarmed
```
...
`verify_bypass_switch.py` is the odd one out: *Bypass Switch* has no render to
compare a preview against — the Python is a stub and the whole node is graph
manipulation. What it tests instead is the manipulation, because every failure
mode there is silent and destructive to somebody's template. The extension is
evaluated whole against a stubbed canvas, DOM and graph, and driven the way the
browser drives it: that the state normaliser is *total* (the preset JSON gets
hand-edited and carried between machines, and a throw inside `configure()`
takes the whole graph load down with it); that a backdrop resolves live, so a
node dragged into it later is covered; that deleted targets are skipped rather
than fatal; that exclusive-group siblings are switched off *before* the winner
is switched on, which is what decides whether a node both of them control ends
up enabled or bypassed; that the widget order follows the preset order with a
group drawing once at its first member; that opening a workflow does not
rewrite node modes; and that a rebuilt widget takes the new state instead of
the value the frontend's store is still holding under that name. The stub store
reproduces that last defect deliberately — it is the one that fires on every
Ctrl+Z.
