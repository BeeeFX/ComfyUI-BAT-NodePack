// Self-contained fork of KJNodes' editor_base.js for the BAT Points
// Editor. The only behavioural difference from upstream is the
// Reset-canvas fix: the original implementation re-uploads a
// downscaled (maxDisplayDim=1024) copy of the bg image as imgData,
// then on Reset reloads that downscaled copy through processImage,
// which clobbers coordWidth/coordHeight with the downscaled dims.
...
export class BaseEditorCanvas {
  ...
  getLocalMouse(e) {
    ...
    // CSS-pixel mapping: clientWidth/clientHeight are the layout-box size
    // (zoom-independent, DPR-independent). rect.width includes LiteGraph's
    // CSS zoom transform. The quotient is the per-CSS-pixel scale.
    // Draws run in CSS pixels too (see ctx.setTransform in _applyDprTransform
    // / beginRender), so hit-testing here matches what's drawn.
    const canvasScaleX = this.canvas.clientWidth  / rect.width;
    const canvasScaleY = this.canvas.clientHeight / rect.height;
  ...
  // Set bitmap to logical-CSS-size × devicePixelRatio while pinning the
  // element's CSS size to the logical size. Without the explicit
  // style.width/height, bumping canvas.width by DPR would also bump the
  // element's intrinsic CSS box (canvas attributes double as CSS pixels
  // when no CSS rule overrides), doubling its on-screen size on hi-DPI.
  // Capped at 2× to keep canvas memory bounded. See also bat_ref_aligner.js
  // and bat_crop.js for the same zoom-stable pattern (different layout).
  _applyDprSize() {
    const dpr = Math.max(1, Math.min(window.devicePixelRatio || 1, 2));
    const cssW = this.width + this.margin * 2;
    const cssH = this.height + this.margin * 2;
  ...
  beginRender() {
    const ctx = this.ctx;
    // Reset transform to CSS-pixel space at the top of every frame. ctx.save
    // captures the current transform, so subclasses that call begin/save/...
    // /restore still see the DPR scale on later draws within the same frame.
  ...
  // handleImageLoad — coord-space dims come from `img.width/height` by default,
  // but a caller can pass `coordOverride: {width, height}` to use those instead
  // (used by _reloadBgImage to honour stored origWidth/origHeight when the
  //  cached bg image is the downscaled version).
  handleImageLoad = (img, downscaledImg, coordOverride = null) => {
    const oldCoordW = this.coordWidth, oldCoordH = this.coordHeight;
    const coordW = coordOverride?.width ?? img.width;
    const coordH = coordOverride?.height ?? img.height;
    this.coordWidth = coordW;
    this.coordHeight = coordH;
    this.widthWidget.value = coordW;
    this.heightWidget.value = coordH;
    if (oldCoordW && oldCoordH && (oldCoordW !== coordW || oldCoordH !== coordH)) {
    ...
    let displayW = coordW, displayH = coordH;
  ...
  processImage = (img, { resize = true } = {}) => {
    ...
    if (embed) {
      ...
      this.node.properties.imgData = { type: 'image/webp', base64: base64String, origWidth, origHeight };
      onStored();
    } else {
      canvas.toBlob((blob) => {
        if (gen !== this._uploadGeneration) return;
        ...
        fetch('/upload/image', { method: 'POST', body: formData })
          ...
          .then(result => {
            if (gen !== this._uploadGeneration) return;
            this.node.properties.imgData = { type: 'temp', filename: result.name, origWidth, origHeight };
            onStored();
          })
  ...
  static setupNode(node, nodeData, config) {
    ...
    // Reload the cached background image into the editor.
    // alignToImage:
    //   true  → resize node to fit the image first (legacy "Align to image" button)
    //   false → keep the current node size and fit the image inside (Reset canvas)
    //
    // FIX vs upstream: when imgData carries origWidth/origHeight (always true
    // for images that went through processImage on this fork), pass them as
    // coordOverride so the coord space restores the ORIGINAL dimensions, not
    // the downscaled cache dims. Without this, every Reset canvas snaps the
    // widgets back to ≤1024 on the longest side.
    const _reloadBgImage = (alignToImage) => {
      ...
      img.onload = () => {
        ...
        const hasOrig = imgData.origWidth && imgData.origHeight;
        if (alignToImage) {
          const targetW = hasOrig ? imgData.origWidth : img.width;
          const savedSize = [node.size[0], node.size[1]];
          node.setSize([Math.min(targetW, maxDisplayDim) + 45, savedSize[1]]);
        }

        if (hasOrig) {
          // Skip processImage's downscale+upload round-trip — the cached
          // file IS the downscaled copy. Use it as bgImage directly and
          // tell handleImageLoad the real coord-space dims.
          node.editor.handleImageLoad(img, img, { width: imgData.origWidth, height: imgData.origHeight });
        } else {
          // Old imgData without origWidth/origHeight (e.g. migrated workflow)
