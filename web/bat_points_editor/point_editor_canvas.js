// Self-contained fork of KJNodes' point_editor_canvas.js for the
// BAT Points Editor. Targets the Bat_PointsEditor server-side class
// instead of PointsEditor. Logic is unchanged from upstream — the
// only behavioural fix lives in the forked editor_base.js
// (origWidth/origHeight preservation on Reset canvas).
...
createEditorStylesheet('bat-pointseditor-stylesheet', 'bat-points-editor');

app.registerExtension({
  name: 'BATNodePack.PointEditor',

  async beforeRegisterNodeDef(nodeType, nodeData) {
    if (nodeData?.name === 'Bat_PointsEditor') {
      chainCallback(nodeType.prototype, "onNodeCreated", function () {
        BaseEditorCanvas.setupNode(this, nodeData, {
          editorClass: BatPointsEditor,
...
class BatPointsEditor extends BaseEditorCanvas {
  ...
  // Small overlay slider (top-right of the canvas) that scales the visual +
  // hit-test radius of the point handles. Does not affect output.
  _addHandleScaleSlider() {
    const parent = this.canvas?.parentNode;
    if (!parent) return;
    parent.querySelector('.bat-pe-handle-slider')?.remove();
    if (getComputedStyle(parent).position === 'static') parent.style.position = 'relative';

    const wrap = document.createElement('div');
    wrap.className = 'bat-pe-handle-slider';
    wrap.style.cssText = 'position:absolute;top:6px;right:8px;z-index:5;display:flex;align-items:center;gap:6px;font:11px monospace;color:#cdd;background:rgba(0,0,0,0.55);padding:3px 8px;border-radius:4px;';
    const slider = document.createElement('input');
    slider.type = 'range'; slider.min = '30'; slider.max = '300'; slider.step = '5';
    slider.value = String(Math.round((this.handleScale || 1) * 100));
    slider.style.width = '90px';
    // Don't let slider interaction start a node drag / canvas pan.
    slider.addEventListener('pointerdown', (e) => e.stopPropagation());
    slider.addEventListener('input', () => {
      this.handleScale = Math.max(0.3, Math.min(3, parseInt(slider.value, 10) / 100));
      localStorage.setItem('bat-pe-handle-scale', String(this.handleScale));
      this.render();
    });
  ...
  findPointAt(x, y, pointsArray) {
    const r = this.pointRadius * (this.handleScale || 1) / Math.min(this.scaleX, this.scaleY);
  ...
  _drawPoints(ctx, points, color, selectedColor, type) {
    const r = this.pointRadius * (this.handleScale || 1);
